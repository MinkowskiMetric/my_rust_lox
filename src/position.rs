...
impl FilePos {
    ...
    pub fn advance_col(&mut self) { self.advance_col_by(1); }
    pub fn advance_col_by(&mut self, by: u32) { self.column += by; }
...
impl<'a> Position<'a> {
    pub fn new(file_name: &'a str, start: FilePos, end: Option<FilePos>) -> Self {
        Self { file_name, start, end }
    }

    pub fn file_name(&self) -> &'a str { self.file_name }
    pub fn start(&self) -> &FilePos { &self.start }
    pub fn end(&self) -> &Option<FilePos> { &self.end }
}
...
pub fn tag_position<'a, T>(t: T, file_name: &'a str, start: FilePos, end: Option<FilePos>) -> PositionTagged<'a, T> {
    PositionTagged(t, Position {
        file_name,
        start,
        end,
    })
